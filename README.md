...
## Running the game
Currently there is only a game executable for Linux (Windows will be coming soon!). To install FreeForm Minesweeper on Linux, download and extract the `FreeForm-Minesweeper-Linux.zip` file and run `sudo ./installer_linux` in the extracted folder from the Releases page. Do NOT run the source Python code of the installer. This will not work.
To play FreeForm Minesweeper on Windows, run `python freeform_minesweeper.py` from the command line, with Python version 3.9.0 or greater and Pillow version 8.2.0 or greater.
