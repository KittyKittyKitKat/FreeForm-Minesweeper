...
## Running the game
Currently there are no game executables (those will be coming soon!) while the game is in its beta stage. To play FreeForm Minesweeper, just run `python freeform_minesweeper.py` from the command line, with Python version 3.9.0 or greater and Pillow version 8.2.0 or greater.
