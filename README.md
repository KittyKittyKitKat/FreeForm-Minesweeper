...
## Game Settings
When you click on the options button, there are a few settings you can tweak. First are the number of rows and columns of the game space. You can theoretically set these to whatever you desire, but 30 by 28, or dimensions close to that, is recommended. Be warned that if you've saved a board that is too large for a smaller game space, it won't load in. Second is the Grace Rule. In Minesweeper, you have the possibility of hitting a mine on your first move. The Grace Rule prevents this from happening and guarantees that you're first move will always be safe. This is on by default, but you can turn it off if you desire. Then there is MultiMine mode. This is a setting, off (Normal mode) by default, that changes the game you're playing in a fun and exciting way. Read below for more information about MultiMine mode. Lastly, there is the MultiMine mode density buff. This controls how many more mines appear in MultiMine mode, and is set to a 20% increase initially. If you're looking for more a challenge, you can raise it all the to a 60% increase. And if MultiMine mode is too hard, you can lower it all the way to 0%. This setting has no effect in Normal mode.
 ...
 * Switch between Revealing and Flagging modes with Ctrl-S
